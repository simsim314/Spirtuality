...
{
  HR = R * 0.25; 
  return difference(union(Draw0(0).translate([R/2,R/2,HR]), cube([R, R, HR]), cylinder({r: R * r_factor, h:(HR), fn: circle_fn}).translate([0, R/2, 0])), union(Draw0(epsilon_material).translate([R/2,R/2,0]), cylinder({r: R * r_factor + epsilon_material, h:(R), fn: circle_fn}).translate([R, R/2, 0])));
}

function draw1_cube()
{
    return difference(union(DrawLine(0).translate([R/2,R/2,R]), cube([R, R, R]), cylinder({r: R * r_factor, h:(R), fn: circle_fn}).translate([0, R/2, 0])), union(Draw0(epsilon_material).translate([R/2,R/2,0]), cylinder({r: R * r_factor + epsilon_material, h:(R), fn: circle_fn}).translate([R, R/2, 0])));
}
