I've finished the 10 axioms tile set. It's 3d printing friendly - special effort was done to include width of material and 45 degrees (lessons learned from past mistakes). 

Here is the [url=https://openjscad.org/]openjscad[/url] code:
...
function Draw0(ep)
{
    return  difference(cylinder({r:(sR + ep), h:(sH + ep)}), cylinder({r:(sW - ep), h:(sH + ep)}));
}

function DrawLine(ep)
{
    return  cube([2*sR+2*ep, sWcube + 2 * ep, sH]).translate([-sR-ep, -(sWcube)/2-ep, 0]);
}
