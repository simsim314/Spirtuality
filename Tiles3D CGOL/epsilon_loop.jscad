...
function draw0_cube()
{
  return difference(union(Draw0(0).translate([R/2,R/2,TileH]), BCube(R,R,TileH, bevelR), cylinder({r: R * r_factor, h:(TileH), fn: circle_fn}).translate([0, R/2, 0])), union(Draw0(epsilon_material).translate([R/2,R/2,0]), cylinder({r: R * r_factor + epsilon_material, h:(R), fn: circle_fn}).translate([R, R/2, 0])));
}

function draw1_cube()
{
    return difference(union(DrawLine(0).translate([R/2,R/2,TileH]), BCube(R,R,TileH, bevelR), cylinder({r: R * r_factor, h:(TileH), fn: circle_fn}).translate([0, R/2, 0])), union(Draw0(epsilon_material).translate([R/2,R/2,0]), cylinder({r: R * r_factor + epsilon_material, h:(TileH), fn: circle_fn}).translate([R, R/2, 0])));
}
...
function draw_text(txt, ht)
{
     var l = vector_text(0,0,txt);   // l contains a list of polylines to be drawn
    var o = [];
    l.forEach(function(pl) {                   // pl = polyline (not closed)
       o.push(rectangular_extrude(pl, {w: 3, h: ht}));   // extrude it to 3D
    });
    return union(o);
}
...
//return union(
    ...
    //);
    var txts = ["11", "12", "13", "14", "15", "16", "17", "18", "19"];
    var o = [];
    
    for(var i = 0; i < 9; i++)
    {
        epsilon_material = 0.11 + i * 0.01;
        o.push(difference(draw0_cube(), scale(0.07, draw_text(txts[i], R)).translate([1,1.2, TileH - R * 0.05])).translate([1.5 * round((i - 1) / 3) * R, 1.5 * (i % 3) * R, 0])); 
    }
